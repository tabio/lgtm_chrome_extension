(function() {
  ...
  function select_flickr(flickrs) {
  ...
  function flickr_images(flickr, page_num, callback) {
    $.ajaxSetup({async: false});
    $.ajax({
      type: 'GET',
      url: flickr_url,
        ...
        switch (false) {
          case kind !== "f":
            return $.getJSON(chrome.extension.getURL("/config/flickr.json"), function(flickrs) {
              var flickr = select_flickr(flickrs);
              var page_num = select_page_num(flickr);
              flickr_images(flickr, page_num, callback);
            });
        }
      }
    }
