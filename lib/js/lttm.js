(function() {
  ...
  var google_api_key      = '';
  var google_engine_id    = '';
  var google_url          = 'https://www.googleapis.com/customsearch/v1';
  ...
  function get_google_params(name) {
    return {
      key: google_api_key,
      cx: google_engine_id,
      q: name
  ...
  function tumblr_images(idol_name, callback) {
    $.ajaxSetup({async: false});
    $.ajax({
      type: 'GET',
      url: tumblr_url,
      data: get_tumblr_params(idol_name)
    }).done(function(data){
      var images = [];
  ...
  function google_images(idol_name, callback) {
    $.ajaxSetup({async: false});
    $.ajax({
      type: 'GET',
      url: google_url,
      data: get_google_params(idol_name)
    }).done(function(data){
      var images = [];
      $.each(data.items, function(k, v) {
        var pagemap = v.pagemap;
        if (pagemap.cse_image && pagemap.cse_thumbnail && pagemap.cse_image[0] && pagemap.cse_thumbnail[0]) {
          var url = pagemap.cse_image[0].src;
          return images.push({
            name: url,
            imageUrl: url,
            imagePreviewUrl: previewUrl(pagemap.cse_thumbnail[0].src),
            alt: 'LGTM:' + idol_name
          });
        }
      });
      ...
      remote_filter: function(query, callback) {
        ...
        switch (false) {
          ...
          case kind !== 'g':
            return $.getJSON(chrome.extension.getURL('/config/idol.json'), function(idols) {
              var idol = select_idol(idols);
              google_images(idol.name, callback);
            });
            break;
          case kind !== 't':
            return $.getJSON(chrome.extension.getURL('/config/idol.json'), function(idols) {
              var idol = select_idol(idols);
              tumblr_images(idol.name, callback);
            });
            break;
        }
      }
